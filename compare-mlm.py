...
from transformers import AutoTokenizer, BertConfig, BertForSequenceClassification, BertForMaskedLM
...
# Example Dataset class for masked language modeling
...
class BertMLM(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        accuracy = accuracy_score(
            ...
            outputs.logits.argmax(-1).flatten().cpu().numpy(),
        )
    ...
    def validation_epoch_end(self, outputs):
        ...
        avg_accuracy = torch.tensor([x["val_accuracy"] for x in outputs]).mean()
    ...
    def test_epoch_end(self, outputs):
        ...
        avg_accuracy = torch.tensor([x["test_accuracy"] for x in outputs]).mean()
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(name=run_name, project="MLM - v1")
    ...
    # print(config)
    # exit(0)
    _model = BertForMaskedLM(config)

    # Modify learning rate
    # https://stackoverflow.com/questions/42966393/is-it-good-learning-rate-for-adam-method
    model = BertMLM(_model, tokenizer=tokenizer, lr=3e-4)
