...
from ..datasets.cellpassport.load_cell_passport import (
    ...
    extract_cell_line_annotation_from_vcf_file, read_vcf)
...
from ..datasets.gwas.load_gwas_catalogue import (download_file,
                                                 extract_snp_details,
                                                 get_risk_snps,
                                                 get_summary_stats_for_snp)
from ..datasets.maves.load_maves import get_maves
from ..datasets.olida.load_olida import (get_variant_combinations,
...
from ..sequence_extractor import (FastaStringExtractor,
                                  GenomeSequenceExtractor,
                                  RandomSequenceExtractor)
from ..utils import read_jsonl, save_as_jsonl
...
class ClinVarDataWrapper:
    def __init__(self, num_records=30000, all_records=True, my_subset=DISEASE_SUBSET, percent=100):
        self.clinvar_vcf_path = load_clinvar.download_file()
        self.records = load_clinvar.read_vcf(self.clinvar_vcf_path,
                                             num_records=num_records,
                                             all_records=all_records)
        self.genome_extractor = GenomeSequenceExtractor()
        self.my_subset = my_subset
    ...
    def get_data(self, Seq_length=20, target='CLNSIG', disease_subset=False):
        ...
        for record in tqdm(self.records[:_records]):
            ref,alt = self.genome_extractor.extract_sequence_from_record(record, sequence_length=Seq_length)
            if ref is None:
                continue
        ...
        if disease_subset:
            ...
            for i in range(len(data)):
                if num_record_with_dis_subset == 0:
                    break
                if data[i][1] not in self.my_subset:
