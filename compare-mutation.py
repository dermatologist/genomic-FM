...
def process_data(data):
    ...
    for x, y in tqdm(data, desc="Appending sequences"):
        ...
        # Get x and y from the random record
        rand_x, _ = random_record
        # get 0 or 1 randomly
        label = random.choice([0, 1])
        if label == 0:
            processed_data.append([x[0], rand_x[0], x[2], label])
        else:
            processed_data.append([x[0], mutate(rand_x[0], 1), x[2], label])
...
def get_df(seq_length=512):
    file_path = f'output/mutation_{seq_length}.pkl'
